...
func (data *Data) FilePath(linkType string, args ...string) (s string) {
	{
		a := make([]string, len(args))
		for i, arg := range args {
			a[i] = url.PathEscape(arg)
		}
		args = a
	}
retry:
	...
	case "index":
		s = "index" + FileExt
	case "resource":
		s = filepath.Join(data.Settings.Output.Resources, filepath.Join(args...))
	case "updates":
		if len(args) > 0 {
			s = filepath.Join("updates", args[0]+FileExt)
		} else {
			s = "updates" + FileExt
		}
	case "class":
		s = filepath.Join(ClassPath, args[0]+FileExt)
	case "member":
		if len(args) == 1 {
	...
	case "enum":
		s = filepath.Join(EnumPath, args[0]+FileExt)
	case "enumitem":
		if len(args) == 1 {
			return (&url.URL{Fragment: MemberAnchorPrefix + args[0]}).String()
		} else if len(args) == 2 {
	...
	case "type":
		...
		s = filepath.Join(TypePath, args[1]+FileExt)
	case "about":
		s = "about" + FileExt
	case "repository":
	...
	s = filepath.Join(data.Settings.Output.Root, data.Settings.Output.Sub, s)
	return s
}

// FileLink generates a URL, relative to an arbitrary host.
func (data *Data) FileLink(linkType string, args ...string) string {
	{
		a := make([]string, len(args))
		for i, arg := range args {
			a[i] = url.PathEscape(arg)
	...
	return data.LinkFromPath(data.FilePath(linkType, args...))
}
